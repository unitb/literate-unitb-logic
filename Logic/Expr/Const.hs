...
fun2' :: AbsFun n a 
      ...
      -> GenExpr n t a q 
      -> GenExpr n t a q
fun2' f t x y       = FunApp f [x,y] t

fun1' :: AbsFun n a 
      -> t
      -> GenExpr n t a q
      -> GenExpr n t a q
fun1' f t x       = FunApp f [x] t
...
not_fun :: (TypeSystem t,IsName n) => AbsFun n t
not_fun = mk_fun [] [smt|not|] [bool] bool

znot :: (IsName n,TypeSystem t,TypeSystem2 a) 
     => GenExpr n t a q
     -> GenExpr n t a q
znot e = case e of 
            FunApp f [x] _
                | f == not_fun -> x
                | otherwise    -> fun1' not_fun bool e
            e' -> fun1' not_fun bool e'
    -- znot         = fun1 mznot
zimplies :: (TypeSystem2 t, IsQuantifier q,IsName n,TypeSystem2 a) 
         => TwoGenExpr n t a q
zimplies x y = runIdentity $ mzimplies' (liftA2 $ fun2' implies_fun bool) (pure x) (pure y)
...
zeq x y = provided (type_of x == type_of y) $ funApp (zeq_fun $ type_of x) [x,y] bool
...
zall xs'      = 
        case xs of
            ...
            xs  
                ...
                | otherwise -> applyAssoc and_fun bool xs
    where
        xs = concatMap f $ F.toList xs'
        f (FunApp fun@(Fun [] n _ _ _ _) xs _)
            | render n == "and" 
...
zsome xs'      = 
        case concatMap f xs of
            ...
            xs
                | ztrue `elem` xs -> ztrue
                | otherwise        -> applyAssoc or_fun bool xs
    where
        xs = toList xs'
        f (FunApp fun@(Fun [] n _ _ _ _) xs _) 
            | n == [smt|or|]
        ...
        f x
            | x == zfalse = []
            | otherwise   = [x]
zforall :: (TypeSystem2 t, IsQuantifier q,IsName n,TypeSystem2 a)
        => [AbsVar n t] 
        -> TwoGenExpr n t a q
...
znothing   = Right $ Cast CodeGen (funApp (mk_fun [] [smt|Nothing|] [] t) [] t) t t
    where
        t = maybe_type gA
...
mzimplies' :: (Monad m,TypeSystem2 t,IsName n,IsQuantifier q,TypeSystem2 a)
...
zopp :: (IsName n,TypeSystem a,TypeSystem t)=> GenExpr n t a q -> GenExpr n t a q
zopp         = fun1' (mk_fun [] [smt|-|] [int] int) int
...
conjuncts (FunApp f xs _) 
    | z3_name f == [smt|and|] = xs
conjuncts x = [x]

disjuncts :: (IsName n,TypeSystem2 t) => AbsExpr n t q -> [AbsExpr n t q]
disjuncts (FunApp f xs _)
    | z3_name f == [smt|or|] = xs
    --  name f == "=>"  = map znot (take 1 xs) ++ drop 1 xs
disjuncts x = [x]

applyAssoc :: (Int -> AbsFun n a) -> t -> [GenExpr n t a q] -> GenExpr n t a q
applyAssoc op t xs = funApp (op (length xs)) xs t
...
flattenConnectors = f . rewrite flattenConnectors
    where
        f e@(FunApp fn xs _) 
            | z3_name fn == [smt|or|]  = applyAssoc or_fun bool $ concatMap disjuncts xs
            | z3_name fn == [smt|and|] = applyAssoc and_fun bool $ concatMap conjuncts xs
            | otherwise                = e
        f e = e

zlift :: t -> AbsExpr n t q -> AbsExpr n t q
zlift t e = Lift e t t
...
zelemUnchecked x s = provided (set_type (type_of x) == type_of s) $
        funApp (elem_fun (type_of x)) [x,s] (type_of x)

zident :: ExprP
zident = Right $ funApp ident_fun [] (type_of (ident_fun :: Fun))
